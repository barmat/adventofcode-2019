...
def manhattan_distance(x, y, start_x=0, start_y=0):
...
def wire_path_coords(wire_path):
	...
	for step in wire_path:
		...
		for i in range(1, steps + 1):
			if direction == 'R':
				coords.append((current_point[0] + i, current_point[1]))
			if direction == 'L':
				coords.append((current_point[0] - i, current_point[1]))
			if direction == 'U':
				coords.append((current_point[0], current_point[1] + i))
			if direction == 'D':
				coords.append((current_point[0], current_point[1] - i))
	return coords

def find_closest_wire_intersection(wire_path1, wire_path2):
	coords1 = wire_path_coords(wire_path1)
	coords2 = wire_path_coords(wire_path2)
	# Make a dict here for fast lookups over large lists of coordinates
	coords2_dict = {}
	for x,y in coords2:
		coords2_dict["%s,%s" % (x, y)] = 1
	intersections = [coords for coords in coords1 if "%s,%s" % (coords[0], coords[1]) in coords2_dict]
	...
	return sorted([manhattan_distance(coords[0], coords[1]) for coords in intersections])[0]

def print_wire_path(wire_path):
	coords = wire_path_coords(wire_path)
	fig = plt.figure()
	ax = fig.add_subplot(111)
	x_points = [x for x,y in coords]
	y_points = [y for x,y in coords]
	p = ax.plot(x_points, y_points, 'b', linewidth=4)
	ax.set_xlabel('x-points')
	ax.set_ylabel('y-points')
	ax.set_title('Wire path')
...
def print_wire_paths(wire_path1, wire_path2):
	coords = wire_path_coords(wire_path1)
	fig = plt.figure()
	ax = fig.add_subplot(111)
	...
	coords = wire_path_coords(wire_path2)
	x_points = [x for x,y in coords]
	y_points = [y for x,y in coords]
	p = ax.plot(x_points, y_points, 'r', linewidth=4)
...
if __name__ == '__main__':
	...
	assert(manhattan_distance(3, 3) == 6)
	assert(manhattan_distance(12, 12) == 24)
	assert(manhattan_distance(12, -12) == 24)
	assert(manhattan_distance(12, 0) == 12)
	assert(manhattan_distance(12, -1) == 13)
	assert(manhattan_distance(0, 0) == 0)
	
	assert(find_closest_wire_intersection('R8,U5,L5,D3', 'U7,R6,D4,L4') == 6)
	assert(find_closest_wire_intersection('R75,D30,R83,U83,L12,D49,R71,U7,L72', 'U62,R66,U55,R34,D71,R55,D58,R83') == 159)
	assert(find_closest_wire_intersection('R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51', 'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7') == 135)
	
	print 'Day 3 tests pass.'
	print 
	print find_closest_wire_intersection(inputs[0], inputs[1])
	
	# print_wire_path('R75,D30,R83,U83,L12,D49,R71,U7,L72')
	# print_wire_paths('R8,U5,L5,D3', 'U7,R6,D4,L4')
	# print_wire_paths('R75,D30,R83,U83,L12,D49,R71,U7,L72', 'U62,R66,U55,R34,D71,R55,D58,R83')
	# quit()
